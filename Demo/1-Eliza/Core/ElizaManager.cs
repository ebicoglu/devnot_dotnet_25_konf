...
namespace Eliza.Core
{
    ...
    public class ElizaManager
    {
        ...
        public ElizaManager() : this(new LoadScriptFromEmbeddedResources())
        {
        }

        public ElizaManager(ILineSource linesource) => ReadScript(linesource);
        ...
        private void Collect(string s)
        {
            ...
            if (EString.Match(s, "*reasmb: *", lines))
            {
                if (lastReasemb == null)
                {
                    ConsoleHelper.WriteLine("Error: no last reasemb");
                    return;
                }
            ...
            else
            {
                if (EString.Match(s, "*decomp: *", lines))
                {
                    if (lastDecomp == null)
                    {
                        ConsoleHelper.WriteLine("Error: no last decomp");
                        return;
                    }
                ...
                else
                {
                    if (EString.Match(s, "*key: * #*", lines))
                    {
                        ...
                        if (lines[2].Length != 0)
                        {
                            ...
                            {
                                ConsoleHelper.WriteLine("Number is wrong in key: " + lines[2]);
                            }
                        }
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    ConsoleHelper.WriteLine("Unrecognized input: " + s);
                                                }
                                            }
                                        }
                                    }
        ...
        private string Assemble(Decomp d, string[] reply, Key gotoKey)
        {
            ...
            if (EString.Match(rule, "goto *", lines))
            {
                ...
                ConsoleHelper.WriteLine("Goto rule did not match key: " + lines[0]);
                return null;
            }
            ...
            while (EString.Match(rule, "* (#)*", lines))
            {
                ...
                {
                    ConsoleHelper.WriteLine("Number is wrong in reassembly rule " + lines[1]);
                }
                if (n < 0 || n >= reply.Length)
                {
                    ConsoleHelper.WriteLine("Substitution number is bad " + lines[1]);
                    return null;
                }
        ...
        private int ReadScript(ILineSource linesource)
        {
            ...
            {
                ConsoleHelper.WriteLine("There was a problem reading the script file.");
                ConsoleHelper.WriteLine("Tried " + linesource.ToString());
                return 1;
            }
            return 0;
        }
